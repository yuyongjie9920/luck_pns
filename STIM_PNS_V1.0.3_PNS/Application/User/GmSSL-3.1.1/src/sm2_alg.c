...
void sm2_fn_mul(SM2_BN ret, const SM2_BN a, const SM2_BN b)
{
	...
	//log("zl = "); for (i = 8; i >= 0; i--) log("%08x", (uint32_t)zl[i]); log("\n");
	//log("zh = "); for (i = 8; i >= 0; i--) log("%08x", (uint32_t)zh[i]); log("\n");
	...
	//log("q  = "); for (i = 7; i >= 0; i--) log("%08x", (uint32_t)q[i]); log("\n");
	...
	//log("qn = "); for (i = 8; i >= 0; i--) log("%08x ", (uint32_t)q[i]); log("\n");
	...
	while (sm2_bn_cmp(r, SM2_N) >= 0) {
		sm2_bn_sub(r, r, SM2_N);
		//log("r-n = "); for (i = 7; i >= 0; i--) log("%16llx ", r[i]); log("\n");
	}
	sm2_bn_copy(ret, r);
}
...
void sm2_jacobian_point_dbl(SM2_JACOBIAN_POINT *R, const SM2_JACOBIAN_POINT *P)
{
	...
	sm2_fp_sqr(T1, Z1);		//log("T1 = Z1^2    = "); print_bn(T1);
	sm2_fp_sub(T2, X1, T1);	//log("T2 = X1 - T1 = "); print_bn(T2);
	sm2_fp_add(T1, X1, T1);	//log("T1 = X1 + T1 = "); print_bn(T1);
	sm2_fp_mul(T2, T2, T1);	//log("T2 = T2 * T1 = "); print_bn(T2);
	sm2_fp_tri(T2, T2);		//log("T2 =  3 * T2 = "); print_bn(T2);
	sm2_fp_dbl(Y3, Y1);		//log("Y3 =  2 * Y1 = "); print_bn(Y3);
	sm2_fp_mul(Z3, Y3, Z1);	//log("Z3 = Y3 * Z1 = "); print_bn(Z3);
	sm2_fp_sqr(Y3, Y3);		//log("Y3 = Y3^2    = "); print_bn(Y3);
	sm2_fp_mul(T3, Y3, X1);	//log("T3 = Y3 * X1 = "); print_bn(T3);
	sm2_fp_sqr(Y3, Y3);		//log("Y3 = Y3^2    = "); print_bn(Y3);
	sm2_fp_div2(Y3, Y3);	//log("Y3 = Y3/2    = "); print_bn(Y3);
	sm2_fp_sqr(X3, T2);		//log("X3 = T2^2    = "); print_bn(X3);
	sm2_fp_dbl(T1, T3);		//log("T1 =  2 * T1 = "); print_bn(T1);
	sm2_fp_sub(X3, X3, T1);	//log("X3 = X3 - T1 = "); print_bn(X3);
	sm2_fp_sub(T1, T3, X3);	//log("T1 = T3 - X3 = "); print_bn(T1);
	sm2_fp_mul(T1, T1, T2);	//log("T1 = T1 * T2 = "); print_bn(T1);
	sm2_fp_sub(Y3, T1, Y3);	//log("Y3 = T1 - Y3 = "); print_bn(Y3);
