...
#define     LOG_BUFFER_SIZE    1024*1           /**< log输出缓冲大小 */
#define     LOG_USING_COLOR    0                /**< 是否使用颜色 */
#define     LOG_MAX_NUMBER     1                /**< 允许注册的最大log对象数量 */
#define     LOG_AUTO_TAG       0                /**< 是否自动添加TAG */
#define     LOG_END            "\r\n"           /**< log信息结尾 */
#define     LOG_TIME_STAMP     (sys_get_tick_ms()/1000) /**< 设置获取系统时间戳 */
...
//#define log(fmt, ...) \
//        logFormat_no_end(INFO_TEXT, LOG_INFO,  fmt, ##__VA_ARGS__)


#define rt_kprintf(fmt, ...)  logFormat(INFO_TEXT, LOG_INFO, fmt, ##__VA_ARGS__)
/**
 * @brief 调试log输出
 * 
 * @param fmt 格式
...
extern void my_log(Log *log, const char *fmt, ...);
#define mylog(fmt, ...) \
        my_log(LOG_ALL_OBJ, fmt, ##__VA_ARGS__)
